#!/usr/bin/env python3
"""s12_worktree_task_isolation.py - task/worktree binding for directory isolation."""
...
class EventBus:
    """Append-only lifecycle events for worktrees and bound tasks."""
...
class TaskManager:
    ...
    def get(self, task_id: int) -> str:
        return json.dumps(self._load(task_id), indent=2, ensure_ascii=True)

    def get_raw(self, task_id: int) -> dict[str, Any]:
    ...
    def list_all(self) -> str:
        ...
        for file_path in sorted(self.dir.glob("task_*.json")):
            tasks.append(json.loads(file_path.read_text(encoding="utf-8")))
        if not tasks:
            return "No tasks."
...
class WorktreeManager:
    """Create/list/run/remove git worktrees with a persistent index."""
    ...
    def remove(self, name: str, force: bool = False, complete_task: bool = False) -> str:
        ...
        try:
            ...
            if complete_task and worktree.get("task_id") is not None:
                ...
                before = self.tasks.get_raw(task_id)
    ...
    def keep(self, name: str) -> str:
        ...
        return json.dumps(kept, indent=2, ensure_ascii=True) if kept else f"Error: Unknown worktree '{name}'"
